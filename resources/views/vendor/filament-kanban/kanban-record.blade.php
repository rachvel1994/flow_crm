...
>
    ...
    <div class="flex items-center gap-6 text-xs text-gray-500 dark:text-gray-400">
        <div class="flex items-center gap-1">
            ...
            ფოტო ({{ is_countable($record->images) ? count($record->images) : 0 }})

        </div>
        <div class="flex items-center gap-1">
            <x-heroicon-o-document class="w-4 h-4"/>
            ფაილი ({{ is_countable($record->attachments) ? count($record->attachments) : 0 }})
        </div>
    </div>
    ...
    <div class="flex items-center gap-3">
        <div class="flex -space-x-2">
            ...
            @if($record->assignees->count() > 3)
                <div
                    class="w-8 h-8 flex items-center justify-center text-xs font-semibold rounded-full bg-gray-200 dark:bg-gray-700 border-2 border-white dark:border-gray-900 text-gray-700 dark:text-gray-300">
    ...
    <div class="flex justify-between items-center pt-2 border-t border-gray-100 dark:border-gray-800">
        <span
            class="px-2 py-0.5 text-xs font-semibold rounded-full bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300"
            style="color:  {{ priority_array($record->priority)['color'] }} ">
